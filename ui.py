...
def ui(stdscr, tensor, fn):
    ...
    def rehighlight():
      ...
      def find_loop(ref, depth):
        nonlocal highlighted
        nonlocal version
        if tree.is_loop(ref) and (tree.loop(ref) == drag[0] and version == drag[1]) and highlighted == None:
          highlighted = ref
        if tree.is_loop(ref) and (tree.loop(ref) == drag[0]):
          version += 1
      tree.walk(find_loop)
      assert(highlighted != None), f"found {version} versions and wanted {drag[1]}:\n" + tree.dump()
